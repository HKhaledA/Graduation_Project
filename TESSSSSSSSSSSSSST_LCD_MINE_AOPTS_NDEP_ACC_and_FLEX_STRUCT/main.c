...
u8  Compare_Accs_Data ( ADXL345_Data * Ptr_Accs_Readings, ADXL345_Data * Ptr_Acss_Stored_Data[10]);
...
//		if(  Compare_Flex_Data(&Sensors_Readings, &Stored_Data) )
		if(  Compare_Accs_Data(&Sensors_Readings , &Accs_Stored_Data) )
		{
						switch(Word_index)
						{
						case Letter_A:
...
{
	Word_index=0;
	if  (  		( (Ptr_Accs_Readings->X_Axis - Ptr_Acss_Stored_Data[Word_index]->X_Axis ) >= - X_Angle_Accuracy )
			 && ( (Ptr_Accs_Readings->X_Axis - Ptr_Acss_Stored_Data[Word_index]->X_Axis ) <=   X_Angle_Accuracy )
			 && ( (Ptr_Accs_Readings->Y_Axis - Ptr_Acss_Stored_Data[Word_index]->Y_Axis ) >= - Y_Angle_Accuracy )
			 && ( (Ptr_Accs_Readings->Y_Axis - Ptr_Acss_Stored_Data[Word_index]->Y_Axis ) <=   Y_Angle_Accuracy )
			 && ( (Ptr_Accs_Readings->Z_Axis - Ptr_Acss_Stored_Data[Word_index]->Z_Axis ) >= - Z_Angle_Accuracy )
			 && ( (Ptr_Accs_Readings->Z_Axis - Ptr_Acss_Stored_Data[Word_index]->Z_Axis ) <=   Z_Angle_Accuracy )
		)
	{
		return 1;
...
void Accelerometor_ReadAxis(ADXL345_Address EFF_Address , ADXL345_Data * Ptr_Accs)
{
	uint16 X_Axis_Read;
	uint16 Y_Axis_Read;
	uint16 Z_Axis_Read;
	...
	Ptr_Accs -> X_Axis= (X_Axis_Read);
	Ptr_Accs -> Y_Axis= (Y_Axis_Read);
	Ptr_Accs -> Z_Axis= (Z_Axis_Read);
