...
#define TRUE				(u8)  1
#define FALSE				(u8)  0
#define X_Angle_Accuracy 	(s16) 50
#define Y_Angle_Accuracy 	(s16) 50
#define Z_Angle_Accuracy 	(s16) 50
/*---------------------------------------------------*/
...
LABEL_ACC:
		LCD_SendCommand(1);
		LCD_WriteString_xy(1,1, "XS= ");
		LCD_WriteInteger_xy(5,1,Stored_Data[0].X_Axis[0],4);
		LCD_WriteString_xy(1,2, "X= ");
		LCD_WriteInteger_xy(5,2,Sensors_Readings.X_Axis[0],4);
goto LABEL_ACC;
*/
...
/*
			...
			******************************* TEST ******************************
			...
			TEST:
					...
					x= Sensors_Readings.X_Axis[0];
					y= Sensors_Readings.Z_Axis[0];
					z= Sensors_Readings.X_Axis[0];
...
*/
	...
	//						Ptr_Sensor_Readings= &Stored_Data[_1st_Counter];
							...
							if  (  		( Ptr_Sensor_Readings->X_Axis[0] - Ptr_Stored_Data[_1st_Counter].X_Axis[0] >= - X_Angle_Accuracy )
//										( Ptr_Sensor_Readings->X_Axis[0] - Ptr_Stored_Data[_1st_Counter].X_Axis[0] <=   X_Angle_Accuracy )
/*									 && ( Ptr_Sensor_Readings->Y_Axis[0] == Ptr_Stored_Data[_1st_Counter].Y_Axis[0] )
									 && ( Ptr_Sensor_Readings->Z_Axis[0] == Ptr_Stored_Data[_1st_Counter].Z_Axis[0] )
*/
								)
...
*/
	...
	if ( (X_Axis_Read < 250) && (X_Axis_Read > 200) )
	Ptr_Acc -> X_Axis[0]= 1;

	if ( (Y_Axis_Read < 40) && (Y_Axis_Read > 30) )
	Ptr_Acc -> Y_Axis[0]= 1;

	if ( (Z_Axis_Read < 250) && (Z_Axis_Read > 200) )
	Ptr_Acc -> Z_Axis[0]= 1;
}
